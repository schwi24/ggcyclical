...
stat_density_circular2 <- function(mapping = NULL, data = NULL,
                                  geom = "area", position = "stack",
                                  ...,
                                  period = 2 * pi,
                                  bw = "SJ",
...
StatDensityCircular2 <- ggproto("StatDensityCircular2", Stat,
  ...
  compute_group = function(
    ...
    circular_density <- compute_density_circular2(
      x = data$x, w = data$weight,
      period = period, from = range[1], to = range[2],
...
compute_density_circular2 <- function(x, w, period = 2 * pi, from, to, bw = "SJ", adjust = 1, kernel = "gaussian", n = 512, bounds = c(-Inf, Inf)) {
  ...
  ggplot2:::data_frame0(
    x = dens$x,
    density = 3 * dens$y,
    scaled = dens$y / max(3 * dens$y, na.rm = TRUE),
    ndensity = dens$y / max(3 * dens$y, na.rm = TRUE),
    count = 3 * dens$y * nx,
    n = nx / 3,
    .size = length(dens$x)
  )
