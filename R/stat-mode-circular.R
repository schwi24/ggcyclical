...
StatModeCircular <- ggproto(
  ...
  compute_group = function(
    ...
    circular_mode <- compute_mode_circular(
      x = data$x, w = data$weight,
      period = period, mode = mode, from = range[1], to = range[2],
...
compute_mode_circular <- function(
    x, w, period = 2 * pi, mode = "peak",
  ...
  if (any(is.finite(bounds))) {
    dens <- circular::density.circular(
      x = x, bw = bw, adjust = adjust, kernel = kernel, n = n
    )
    ...
    dens <- circular::density.circular(
      x = x, bw = bw, adjust = adjust, kernel = kernel, n = n,
      from = circular::circular(from), to = circular::circular(to)
    )
  }
  ...
  ggplot2:::data_frame0(
    x = dens$x[sel] * period / (2 * pi),
    density = dens$y[sel] * 2 * pi / period,
    scaled = dens$y[sel] / max(dens$y, na.rm = TRUE),
    ndensity = dens$y[sel] / max(dens$y, na.rm = TRUE),
    count = dens$y[sel] * nx * 2 * pi / period,
    n = nx,
    .size = length(dens$x[sel])
  )
}

stat_mode_circular2 <- function(mapping = NULL, data = NULL,
                               geom = "point", position = "stack",
                               ...,
                               mode = "peak",
                               bw = "SJ",
...
StatModeCircular2 <- ggproto(
  ...
  compute_group = function(
    ...
    period = 2 * pi,
    mode = "peak",
    bw = "SJ", adjust = 1, kernel = "gaussian", n = 512, trim = FALSE,
    na.rm = FALSE, bounds = c(-Inf, Inf),
    flipped_aes = FALSE
  ) {
    ...
    circular_mode <- compute_mode_circular2(
      x = data$x, w = data$weight,
      period = period, mode = mode, from = range[1], to = range[2],
...
compute_mode_circular2 <- function(x, w, period = 2 * pi, mode = "peak", from, to, bw = "SJ", adjust = 1, kernel = "gaussian", n = 512, bounds = c(-Inf, Inf)) {
  
  #Extend circular data ± period
  nx <- 3 * length(x)
  x <- c(x - period, x, x + period)
  if (is.null(w)) {
    w <- rep(1/nx, nx)
  } else {
    w <- rep(w, 3) / (3 * sum(w))
  ...
  sel <- switch(
    mode,
    "peak" = which(diff(sign(diff(3 * dens$y))) < 0) + 1,
    "trough" = which(diff(sign(diff(3 * dens$y))) > 0) + 1,
    "both" = c(which(diff(sign(diff(3 * dens$y))) < 0) + 1, which(diff(sign(diff(3 * dens$y))) > 0) + 1)
  )
  
  ggplot2:::data_frame0(
    x = dens$x[sel],
    density = 3 * dens$y[sel],
    scaled = dens$y[sel] / max(3 * dens$y, na.rm = TRUE),
    ndensity = dens$y[sel] / max(3 * dens$y, na.rm = TRUE),
    count = 3 * dens$y[sel] * nx,
    n = nx / 3,
    .size = length(dens$x[sel])
  )
}